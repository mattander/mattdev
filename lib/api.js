...
export function getPageBySlug(slug, postType = 'post', fields = []) {
    const realSlug =
        slug === 'home' || slug === 'index' ? '' : slug.replace(/\.md$/, '');
    const fullPath = join(getPostTypeDirectory(postType), realSlug, `index.md`);
    const fileContents = fs.readFileSync(fullPath, 'utf8');
